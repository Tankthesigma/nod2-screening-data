#!/usr/bin/env python3
"""
URSODIOL REPLICATE COMPARISON
Rep1 (86% occ) vs Rep2 (28.5% occ) vs Rep3 (53.6% occ)
...
OUTPUT_DIR = Path(r'C:\Users\vasud\nod2-screening-data\PHASE_7_analysis\ursodiol_comparison')
...
POCKET_OCC = {1: 86.0, 2: 28.5, 3: 53.6}
...
def main():
    print("=" * 80)
    print("URSODIOL REPLICATE COMPARISON")
    print("Rep1 (86%) vs Rep2 (28.5%) vs Rep3 (53.6%)")
    print("=" * 80)

    # Load all three replicates
    # Note: ursodiol uses rep3 topology according to COMPOUNDS dict
    reps = {}
    for rep in [1, 2, 3]:
        u = mda.Universe(
            str(TRAJ_DIR / 'ursodiol_rep3_solvated.pdb'),
            str(TRAJ_DIR / f'ursodiol_rep{rep}.dcd')
        )
    ...
    # Compute pocket center using ursodiol universe (same protein)
    ...
    colors = ['#2ca02c', '#d62728', '#ff7f0e']  # Green for best, red for worst, orange for middle
    ...
    plt.suptitle('URSODIOL: Why did Rep1 (86%) outperform Rep2 (28.5%) and Rep3 (53.6%)?',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'ursodiol_replicate_comparison.png', dpi=300)
    plt.close()
    print(f"\nSaved: {OUTPUT_DIR / 'ursodiol_replicate_comparison.png'}")
    ...
    for rep in [1, 2, 3]:
        ...
        pdb_file = OUTPUT_DIR / f'ursodiol_rep{rep}_frame0.pdb'
    ...
    # =========================================================================
    print("\n" + "=" * 80)
    print("*** FINAL VERDICT: WHY DID REP1 PERFORM BEST? ***")
    print("=" * 80)
    ...
    for rep in [1, 2, 3]:
        status = "BEST" if rep == 1 else ("WORST" if POCKET_OCC[rep] == min(POCKET_OCC.values()) else "MIDDLE")
    ...
    if hbond_data[best_rep]['count'] > hbond_data[worst_rep]['count']:
        print(f"\n  Rep{best_rep} succeeded likely due to MORE H-BONDS at start")
    if contact_data[best_rep]['count'] > contact_data[worst_rep]['count']:
        print(f"  Rep{best_rep} had BETTER pocket coverage ({contact_data[best_rep]['count']}/8 vs {contact_data[worst_rep]['count']}/8)")
    if frame0_data[best_rep]['dist_to_pocket_center'] < frame0_data[worst_rep]['dist_to_pocket_center']:
        print(f"  Rep{best_rep} started CLOSER to pocket center")

    print("""
For your ISEF report:
  - Ursodiol shows moderate binding variability across replicates
  - Rep1 demonstrates stable binding is achievable
  - Initial pose and H-bond network influence long-term stability
""")
    print("=" * 80)
