#!/usr/bin/env python
"""Canary test for windows 15-19 - WSL/CUDA version."""
import numpy as np
import time
...
EQUIL_STEPS = 10000    # 20 ps equilibration
...
def run_window_canary(sys_name, window_idx):
    ...
    # Create integrator and simulation with CUDA
    ...
    try:
        ...
        properties = {}
        print(f"    Using CPU platform (CUDA unavailable)")
    ...
    print(f" done ({eq_time:.1f}s, {EQUIL_STEPS/eq_time:.0f} steps/s)")
    ...
    print(f" done ({prod_time:.1f}s, {PROD_STEPS/prod_time:.0f} steps/s)")
...
def main():
    print("="*60)
    print("CANARY TESTS - Windows 15-19 (WSL/CUDA)")
    print("="*60)
